...
<body>
<%
String sType_Invoice = request.getParameter("invoice_no_for_service");
String hotel_id = request.getParameter("s_hotelID");


%>

<%!
public List<String> getServiceTypes(String sType_Invoice){
	...
	Statement getSTypes = con.createStatement();
	String sTypesString = "SELECT Reservation_Contains.sType, Reservation_Contains.HotelID FROM Reservation_Contains WHERE Reservation_Contains.InvoiceNo = "+sType_Invoice;
    ResultSet sType = getSTypes.executeQuery(sTypesString);
   
	List<String> sTypeNames = new ArrayList<String>();
	while(sType.next()){
		String temp = sType.getString("Reservation_Contains.sType");
		sTypeNames.add(temp);
		String hotel_id = sType.getString("Reservation_Contains.HotelID");
	}
	con.close();
	return sTypeNames;
	
	}catch(Exception e){
		System.out.println("Error getting Service Types");
		return null;
	}
}
%>
  ...
  <form class="jumbotron" method = "selectedHotel" action = "store_service_review.jsp">
    ...
    <div class="form-group col-2 ">
      ...
      <select class="form-control" id="sType_review" name="sType">
        ...
        int count = 0;
    	while(count<sTypeNames.size()){
    		out.print("<option value = \""+sTypeNames.get(count)+"\">"+ sTypeNames.get(count)+"</option>");
    		count++;
    	}
        %>
      </select>
...
<div id = "HotelID" class="hidden"></div>
</form>
